...
int main(int argc, char** argv) {
    ...
    const vrp::app::Reporter reporter(std::cout, options.params.generations, 10,
                                      options.quiet);
    // An absent callback under --quiet, not a callback that returns early.
    // Solver evaluates population.fitness(population.bestIndex()) -- an
    // O(populationSize) scan, inside the timed region -- only when a callback
    // is installed. Handing it a reporter that discards the value would still
    // pay to produce it: 10M comparisons charged to elapsedSeconds at the
    // defaults. --quiet is the benchmark path, so it must cost nothing.
